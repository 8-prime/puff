...
fn runUnPuff(args: [][:0]u8, alloc: std.mem.Allocator, output: std.io.AnyWriter) !void {
    if (args.len < 3) {
        try output.print("Must specify at least one input file with -i (--input)\n", .{});
        std.process.exit(1);
    }
    ...
    const sure_in_file = in_file orelse {
        try output.print("Must specify input file with -i (--input)\n", .{});
        std.process.exit(1);
    };

    const sure_out_file = out_file orelse {
        try output.print("Must specify output file with -o (--output)\n", .{});
        std.process.exit(1);
    };
    try puff.unPuff(alloc, sure_in_file, sure_out_file);
}
...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, args[1], "puff")) {
        ...
        try runUnPuff(args, alloc, outw.any());
    }
}
